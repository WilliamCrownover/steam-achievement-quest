...
export const AchievementSortOrder = (props: PassDownSteamData) => {
	const { userData, gamesWithAchievements, setGamesWithAchievements } = props;
		...
		setGamesWithAchievements(
			gamesWithAchievements.map(game => {
				const sortProperty = (property: string) => {
					...
					const achievementsSortedByPercent: CombinedAchievementsWithSchema[] = sorter(
						achievements,
						sortNumber('percent', true)
					);
					switch (property) {
						case 'name':
