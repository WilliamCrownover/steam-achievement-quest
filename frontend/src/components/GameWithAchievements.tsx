...
export const GameWithAchievements = (props: GameWithAchievementsProps) => {
	...
	const oneHundredPercent = percentComplete === 100.0 ? 'oneHundredPercent' : '';
		...
		<div className={`gameWithAchievementsContainer ${getEnumKeyByValue(ReviewEnum, review)}`}>
			...
			<div className="multipleForms">
				...
				>
					...
					{!privateProfile && (
						<>
							<h3>
								{totalIncompleteAchievements} Not Completed -{' '}
								{round(100 - percentComplete)}%
							</h3>
							<h3>
								{totalCompletedAchievements} Completed - {percentComplete}%
							</h3>
						</>
					)}
				</a>
			...
			{showGraph && (
				<>
					...
					<h3
						className={`${percentComplete === 100.0 && 'achieved'} averagePercent`}
			...
			{showList && achievements && (
				<AchievementPercentages achievements={achievements} showIcons={showIcons} />
			)}
		</div>
	);
};
