...
export const GameSortOrder = (props: PassDownSteamData) => {
	...
	const countPurchaseDates = (games: GameDataExpanded[]) => {
		...
		Object.keys(purchaseDates).forEach(year => {
			purchaseDatesYearTotals[year] = Object.values(purchaseDates[year]).reduce(
				(acc, count) => acc + count,
				0
			);
		});
	...
	const changeGameOrder = (e: ChangeEvent<HTMLSelectElement>) => {
		...
		switch (true) {
			...
			case value === 'percentComplete':
				sortNumberThenSetState(
					setGamesWithAchievements,
					[...sorter([...gamesWithAchievements], sortNumber('totalAchievements'))],
					value,
					true
				);
				break;
				...
				{hasAchievements && (
					<>
						...
						{!userData.privateProfile && (
							<>
								<option value="totalCompletedAchievements">
									Total Achievements Complete
								</option>
								<option value="totalIncompleteAchievements">
									Total Achievements Incomplete
								</option>
						...
						<option value="lowestAchievementPercent">
							{' '}
							Lowest Achievement Percent per Game
						</option>
					</>
				)}
			</select>
