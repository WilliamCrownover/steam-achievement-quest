...
export const SteamSpyAppDetails = (props: SteamSpyAppDetailsProps) => {
	const { developer, publisher, owners, genre, tags, languages } = props.ssAppDetails;
			...
			{dataType.length > 0 && !dataType.includes('Unknown') && (
				<div className="gameStoreDetailContainer">
					...
					{dataType.map((string, index) => (
						<h4 className="gameStoreDetailDataPoint" key={index}>
							{string}
						</h4>
					))}
				</div>
		...
		<div>
			...
			{owners && (
				<div className="gameStoreDetailContainer">
					...
					<h4 className="gameStoreDetailDataPoint" key={'owners'}>
						{owners}
					</h4>
				</div>
			)}
